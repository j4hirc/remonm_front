...
export class BodegaComponent implements OnInit {
  ...
  constructor() { 
    this.destroyRef.onDestroy(() => {
      this.clearDownload();
      if (Swal.isVisible()) Swal.close(); // Solo cerramos si está visible
    }); 
  }
  ...
  async load(): Promise<void> {
    ...
    try { 
      ...
      Swal.close(); // Lo movemos al finally para garantizar que SIEMPRE se cierre
  ...
  changePeriod(delta: number): void {
    ...
    // CORRECCIÓN: Validar antes de resetear para no romper los botones
    if (Swal.isVisible()) {
  ...
  async exportPdf(): Promise<void> {
    ...
    // CORRECCIÓN: Validar antes de resetear
    if (Swal.isVisible()) {
      Swal.resetValidationMessage();
    }
